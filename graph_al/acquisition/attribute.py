...
class AcquisitionStrategyByAttribute(BaseAcquisitionStrategy):
    ...
    def acquire_one(self, mask_acquired: Bool[Tensor, 'num_nodes'], prediction: Prediction | None, model: BaseModel, dataset: Dataset, 
        ...
        idx_sampled, attribute_info = self.compute_idx_from_metric(attribute, mask_acquired, model, dataset, generator)
        return idx_sampled, attribute_info
    
    def compute_idx_from_metric(self, attribute: Tensor, mask_acquired: Bool[Tensor, 'num_nodes'], model: BaseModel, dataset: Dataset, generator: Generator) -> Tuple[int, Dict[str, Tensor]]:
