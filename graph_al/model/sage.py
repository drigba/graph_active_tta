...
from graph_al.model.config import SAGEConfig
...
class SAGE(BaseModelMonteCarloDropout):
    """ GCN model. """
    
    def __init__(self, config: SAGEConfig, dataset: Dataset):
        ...
        for out_dim in list(config.hidden_dims) + [dataset.data.num_classes]:
            self.layers.append(tgnn.SAGEConv(in_dim, out_dim))
            in_dim = out_dim
    ...
    def forward_impl(self, x: Float[Tensor, 'num_nodes num_input_features'],
        ...
        for layer_idx, layer in enumerate(self.layers[-last_n_layer:]):
            ...
            if acquisition and _layer_idx == len(self.layers) - 2:
                embedding = x
    ...
    def forward(self, batch: Data, acquisition: bool=False) -> Tuple[Float[Tensor, 'num_nodes num_embeddings'] | None, 
        ...
        embeddings, logits = self.forward_impl(batch.x, batch.edge_index, acquisition=acquisition)
        if acquisition:
            embeddings_unpropagated, logits_unpropagated = self.forward_impl(batch.x, edge_index=None, 
