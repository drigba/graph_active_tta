...
class BALDPredictor(EQSPredictor):
    ...
    def predict(
        ...
        with torch.no_grad():
            ...
            # QES
            qes_scores = [
                self.acquisition_strategy.get_attribute(
                    pred_tmp,
                    self.augmentor.model,
                    dataset,
                    self.generator,
                    self.model.config,
                )
                for pred_tmp in augmented_predictions
            ]
            qes_scores = torch.vstack(qes_scores)
            ...
            qes_scores = qes_scores * masks / (masks.sum(dim=0) + 1)
            qes_score = qes_scores.sum(0)
            qes_score += orig_score / (masks.sum(dim=0) + 1)
            ...
            eqs_score = self.acquisition_strategy.get_attribute(
                prediction,
                self.augmentor.model,
                dataset,
                self.generator,
            ...
            bald_score = eqs_score - qes_score
            
            bald_score *= -1
            
            mask_acquired = torch.zeros_like(dataset.data.mask_train_pool)
            acquired_idxs, acquisition_metrics = (
        ...
        return prediction, torch.tensor([acquired_idxs]), acquisition_metrics
