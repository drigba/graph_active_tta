...
def initial_acquisition(predictor: BasePredictor, config: Config, dataset: Dataset, 
    ...
    all_aquired_idxs = []
    predictor.acquisition_strategy.reset()
    for acquisition_step in range(1, 1 + config.initial_acquisition_strategy.num_steps):
        with torch.no_grad():
            _, acquired_idxs, _ = predictor.predict(dataset,0)
